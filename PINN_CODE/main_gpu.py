...
class L_BFGS_B:
    ...
    def print_fields(self, epoch=0):
        ...
        plot_contours(x, y, psi, 'Stream Function ψ', axs[0, 0], epoch)
        plot_contours(x, y, p, 'Pressure p', axs[0, 1], epoch)
        plot_contours(x, y, u, 'Velocity u', axs[1, 0], epoch)
        plot_contours(x, y, v, 'Velocity v', axs[1, 1], epoch)
        plt.tight_layout()
        plt.show()
...
def plot_contours(x, y, z, title, ax, epoch):
    if z.min() == z.max():
    ...
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    plt.savefig(f'PINN_lid/output_{epoch}.png')
...
if __name__ == '__main__':
    ...
    lbfgs = L_BFGS_B(model=pinn, x_train=x_train, y_train=y_train, print_interval=200)
    lbfgs.fit()

    # Save the trained model
    lbfgs.save_model("pinn_model_lid.pth")
