...
config = {
    ...
    'init_type': 'kaiming',
    'alpha': 10,  # Weight for boundary loss
}
...
def generate_structured_boundary_and_internal_points(num_boundary, num_internal):
    ...
    # Remaining part of left wall with zero velocities+one hole
    left_ys = np.linspace(0.2, 0.8, num_left)
    for y in left_ys:
        boundary_points.append([0, y])
    ...
    outlet_ys = np.linspace(0.0, 1, num_right)
    for y in outlet_ys:
        boundary_points.append([1, y])
        boundary_values.append([0, 0])
...
class L_BFGS_B:
    ...
    def print_fields(self, epoch):
        ...
        plt.tight_layout()
        #plt.show()
        plt.savefig(f'New_test_case/swerve_{epoch}.png')
...
class PINN(nn.Module):
    ...
    def loss_fn(self, x_train, y_train):
        ...
        loss_bnd = 10*((uv_bnd - uv_bnd_target) ** 2).mean()
...
if __name__ == '__main__':
    ...
    boundary_points, boundary_values, internal_points = generate_structured_boundary_and_internal_points(num_boundary=2000, num_internal=8000)
    ...
    lbfgs.save_model("new_test_case.pth")
